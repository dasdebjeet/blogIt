$(document).ready(() => {
    ...
    var usrImg_el = (img_add) => {
        ...
        $(".navLogin_btn").remove()
        $(navUser_img).appendTo(".nav_userProfile") // index nav
        $(".dashboard_user_profile").html('<img src="./assests/users_img/' + img_add + '" class="user_img">') // dashboard nav
    }
            ...
            if (response != "New user") {
                if (response == "You logged in from a new device! Please login") { // logined from a new device 
                    ...
                    if (log_newStat != log_oldStat) { // not logined
                        ...
                        $(".navLogin_btn").remove()
                        $(".nav_userProfile>img").remove()
                        $("<div class='navLogin_btn flexc'><div class='water'></div><span>login</span></div>").appendTo(".navLogin_btn_con") // index nav

                        $(".dashboard_user_profile").html('<img src="./assests/users_img/default_profile_img.png" class="user_img">') // dashboard nav
